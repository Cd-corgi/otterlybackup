                ...
                if (choice === "start") {
                    ...
                    if (choice === "add-role") {
                        ...
                        if (choice === "remove-role") {
                            ...
                            if (!menu.roles.some(v => v.role === rr.id)) return interaction.editReply({ content: `Reaction Role menu do not have this role as its part` });

                            menu.roles = menu.roles.filter((v) => v.role !== rr.id);

                            await self_roles.findOneAndUpdate({ name, guild: interaction.guildId }, { roles: menu.roles });

                            interaction.editReply({ content: `Remove role \`${rr.name}\`from menu : \`${menu.name}\`` });
                        }
    }
} //a