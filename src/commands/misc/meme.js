    ...
    async run(client, interaction) {
        fetch("https://api.popcat.xyz/meme").then(response => response.json()).then(async data => {
            const mem = new EmbedBuilder()
                ...
                .setURL(`${data.url}`)
                .addFields({ name: "Meme Title", value: `${data.title}`, inline: true }, { name: "Upvotes", value: `${data.upvotes}`, inline: true }, { name: "Comments 💬", value: `${data.comments}`, inline: true })
                .setImage(`${data.image}`)
                .setFooter({ text: `Powered by ${client.user.username}` })

            await interaction.deferReply();
            await wait(3000)
            await interaction.editReply({ embeds: [mem] })
        }).catch(e => {
